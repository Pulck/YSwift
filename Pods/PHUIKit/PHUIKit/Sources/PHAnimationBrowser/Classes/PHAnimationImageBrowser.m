...
@interface PHAnimationImageBrowser () <UICollectionViewDelegate, UICollectionViewDataSource>

@property (nonatomic, strong) YBIBCollectionView *collectionView;
@property (nonatomic, strong) PHDataMediator *dataMediator;
@property (nonatomic, strong) PHScreenRotationHandler *rotationHandler;

@end

@implementation PHAnimationImageBrowser {
...
- (void)initValue {
    ...
    _toolViewHandlers = @[[PHToolViewHandler new]];
...
- (void)buildToolView {
    for (id<PHToolViewHandler> handler in self.toolViewHandlers) {
...
- (PHDataMediator *)dataMediator {
    if (!_dataMediator) {
        _dataMediator = [[PHDataMediator alloc] initWithBrowser:self];
...
- (PHScreenRotationHandler *)rotationHandler {
    if (!_rotationHandler) {
        _rotationHandler = [[PHScreenRotationHandler alloc] initWithBrowser:self];
    }
