...
NSData * cipherOperation(NSData *contentData, NSData *keyData, CCOperation operation, NSString *initVector) {
    NSUInteger dataLength = contentData.length;

    void const *initVectorBytes = [initVector dataUsingEncoding:NSUTF8StringEncoding].bytes;
...
NSString * aesEncryptString(NSString *content, NSString *key, NSString *initVector) {
    ...
    NSData *encrptedData = aesEncryptData(contentData, keyData, initVector);
...
NSString * aesDecryptString(NSString *content, NSString *key, NSString *initVector) {
    ...
    NSData *keyData = [key dataUsingEncoding:NSUTF8StringEncoding];
    NSData *decryptedData = aesDecryptData(contentData, keyData, initVector);
...
NSData * aesEncryptData(NSData *contentData, NSData *keyData, NSString *initVector) {
    ...
    return cipherOperation(contentData, keyData, kCCEncrypt,initVector);
}
...
NSData * aesDecryptData(NSData *contentData, NSData *keyData, NSString *initVector) {
    ...
    NSString *hint = [NSString stringWithFormat:@"The key size of AES-%lu should be %lu bytes!", kKeySize * 8, kKeySize];
    NSCAssert(keyData.length == kKeySize, hint);
    return cipherOperation(contentData, keyData, kCCDecrypt,initVector);
}
